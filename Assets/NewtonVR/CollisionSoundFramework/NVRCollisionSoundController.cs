...
namespace NewtonVR {
  public class NVRCollisionSoundController : MonoBehaviour {
    ...
    [Tooltip("Don't play collision sounds that will produce an impact with a volume lower than this number")] public
      float MinCollisionVolume = 0.1f;

    [Tooltip("Turns on or off randomizing the pitch of the collision sounds")] public bool PitchModulationEnabled = true;

    [Range(0f, 3f)] public float PitchModulationRange = 0.5f;

    [HideInInspector] public NVRCollisionSoundProviders SoundEngine = NVRCollisionSoundProviders.Unity;

    [Tooltip("The max number of sounds that can possibly be playing at once.")] public int SoundPoolSize = 100;
    ...
    public static void Play(NVRCollisionSoundMaterials material, Vector3 position, float impactVolume) {
      if (Provider != null) Provider.Play(material, position, impactVolume);
    }
  }
