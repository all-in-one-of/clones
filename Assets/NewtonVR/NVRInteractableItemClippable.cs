...
namespace NewtonVR {
  ...
  public class NVRInteractableItemClippable : NVRInteractable {
    ...
    protected void FixedUpdate() {
      if (IsAttached == true) {
        ...
        if (InteractionPoint != null) {
          TargetRotation = AttachedHand.transform.rotation *
                           Quaternion.Inverse(InteractionPoint.localRotation);
          TargetPosition = this.transform.position +
                           (AttachedHand.transform.position - InteractionPoint.position);
        } else {
    ...
    public override void BeginInteraction(NVRHand hand) {
      ...
      PickupTransform =
        new GameObject(string.Format("[{0}] NVRPickupTransform", this.gameObject.name)).transform;
