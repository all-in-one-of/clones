...
public class BorderRenderer : MonoBehaviour {
  ...
  [Tooltip(
    "Material used to render the border mesh.  UV's are set up so that v=0->bottom and v=1->top.  u is stretched along each edge."
  )]
  ...
  private void Update() {
    if (CachedMeshes == null || BorderMaterial == null) return;
    ...
    foreach (Mesh m in CachedMeshes) Graphics.DrawMesh(m, Transpose, BorderMaterial, gameObject.layer, null, 0, null, false, false);
  }
  ...
  private void OnValidate() {
    ...
    if (BorderMaterial != null) BorderMaterial.SetFloat(AlphaShaderID, BorderAlpha);
  }
  ...
  public void RegenerateMesh() {
    ...
    for (int x = 0; x < CachedMeshes.Length; x++) {
      if (Points[x] == null || Points[x].Points == null) CachedMeshes[x] = new Mesh();
      else CachedMeshes[x] = GenerateMeshForPoints(Points[x].Points);
    }
  }

  private Mesh GenerateMeshForPoints(Vector3[] Points) {
    if (Points.Length <= 1) return new Mesh();
