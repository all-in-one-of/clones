...
public class ParabolicPointer : MonoBehaviour {
  ...
  private static Vector3 ParabolicCurve(Vector3 p0, Vector3 v0, Vector3 a, float t) {
    ...
    for (int x = 0; x < 3; x++) ret[x] = ParabolicCurve(p0[x], v0[x], a[x], t);
    return ret;
  }
  ...
  private static Vector3 ParabolicCurveDeriv(Vector3 v0, Vector3 a, float t) {
    Vector3 ret = new Vector3();
    for (int x = 0; x < 3; x++) ret[x] = ParabolicCurveDeriv(v0[x], a[x], t);
    return ret;
  }
  ...
  private static bool CalculateParabolicCurve(Vector3 p0, Vector3 v0, Vector3 a, float dist, int points, ViveNavMesh nav,
                                              List<Vector3> outPts, out Vector3 normal) {
    outPts.Clear();
    outPts.Add(p0);
  ...
  private void OnDisable() {
    if (SelectionPadObject != null) SelectionPadObject.SetActive(false);
    if (InvalidPadObject != null) InvalidPadObject.SetActive(false);
  }
  ...
  private void Update() {
    ...
    Vector3 normal;
    PointOnNavMesh = CalculateParabolicCurve(transform.position, velocity, Acceleration, PointSpacing, PointCount,
      NavMesh, ParabolaPoints, out normal);
  ...
  private float ClampInitialVelocity(ref Vector3 velocity, out Vector3 velocity_normalized) {
    ...
    if (Vector3.Dot(right, Vector3.Cross(velocity_fwd, velocity)) > 0) angle *= -1;
    ...
    if (angle > 45) {
      ...
      angle = 45;
    } else velocity_normalized = velocity.normalized;
  ...
  private void OnDrawGizmos() {
    ...
    if (ParabolaPoints_Gizmo == null) ParabolaPoints_Gizmo = new List<Vector3>(PointCount);
    ...
    Vector3 normal;
    bool didHit = CalculateParabolicCurve(transform.position, velocity, Acceleration, PointSpacing, PointCount, NavMesh,
      ParabolaPoints_Gizmo, out normal);

    Gizmos.color = Color.blue;
    for (int x = 0; x < ParabolaPoints_Gizmo.Count - 1; x++) Gizmos.DrawLine(ParabolaPoints_Gizmo[x], ParabolaPoints_Gizmo[x + 1]);
    Gizmos.color = Color.green;

    if (didHit) Gizmos.DrawSphere(ParabolaPoints_Gizmo[ParabolaPoints_Gizmo.Count - 1], 0.2f);
  }
#endif
}
