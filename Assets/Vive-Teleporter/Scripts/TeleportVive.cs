...
public class TeleportVive : MonoBehaviour {
  ...
  private void Start() {
    ...
    int[] elts = {0, 1, 2, 0, 2, 3};
    PlaneMesh.vertices = verts;
    PlaneMesh.triangles = elts;
    PlaneMesh.RecalculateBounds();

    if (FadeMaterial != null) FadeMaterialInstance = new Material(FadeMaterial);
    ...
    if (GetChaperoneBounds(out p0, out p1, out p2, out p3)) {
      ...
      BorderPointSet p =
        new BorderPointSet(new Vector3[] {
          originRotationMatrix * p0, originRotationMatrix * p1, originRotationMatrix * p2, originRotationMatrix * p3,
          originRotationMatrix * p0,
        });
      RoomBorder.Points = new[] {p};
    }
  ...
  private void Update() {
    ...
    // At this point, we are NOT actively teleporting.  So now we care about controller input.
      ...
      if (shouldTeleport || shouldCancel) {
        ...
        if (NavmeshAnimator != null) NavmeshAnimator.SetBool(EnabledAnimatorID, false);
        ...
        if (IsClicking && Mathf.Abs(angleClickDiff) > HapticClickAngleStep) {
          LastClickAngle = Pointer.CurrentPointVector;
          if (Pointer.PointOnNavMesh) device.TriggerHapticPulse();
        }
        ...
        if (Pointer.PointOnNavMesh && !IsClicking) {
          ...
          LastClickAngle = Pointer.CurrentPointVector;
        } else if (!Pointer.PointOnNavMesh && IsClicking) IsClicking = false;
      }
    } else //CurrentTeleportState == TeleportState.None
    {
      ...
      foreach (SteamVR_TrackedObject obj in Controllers.Select(hand => hand.GetComponent<SteamVR_TrackedObject>())) {
        ...
        if (device.GetPressDown(SteamVR_Controller.ButtonMask.Trigger)) {
          ...
          if (NavmeshAnimator != null) NavmeshAnimator.SetBool(EnabledAnimatorID, true);
