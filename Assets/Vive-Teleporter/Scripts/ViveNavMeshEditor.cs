...
public class ViveNavMeshEditor : Editor {
  ...
  public override void OnInspectorGUI() {
    ...
    if (EditorGUI.EndChangeCheck()) {
      p_area.intValue = 0;
      for (int x = 0; x < areaNames.Length; x++) p_area.intValue |= (((temp_mask >> x) & 1) == 1 ? 0 : 1) << area_index[x];
    ...
    if (isPrefab && mesh.SelectableMesh == null) mesh.SelectableMesh = new Mesh();
    ...
    if (MeshNull || BorderNull) {
      ...
      if (MeshNull) str += "Selectable Mesh == null.  ";
      if (BorderNull) str += "Border point array == null.  ";
      str +=
  ...
  private static void DewarpMesh(Vector3[] verts, NavmeshDewarpingMethod dw, float step) {
    if (dw == NavmeshDewarpingMethod.None) return;
    ...
    for (int x = 0; x < verts.Length; x++) {
      if (dw == NavmeshDewarpingMethod.RaycastDownward) {
        ...
        if (Physics.Raycast(sample, Vector3.down, out hit, step + 0.01f)) verts[x] = hit.point;
  ...
  private static void CullNavmeshTriangulation(Vector3[] vertices, int[] indices, int[] areas, int areaMask,
                                               bool ignore_sloped_surfaces, ref int vert_size, ref int tri_size) {
    ...
    for (int i = 0; i < tri_size; i++) {
      ...
      if (((1 << areas[i]) & areaMask) == 0 || (ignore_sloped_surfaces && !vertical))
        // If true this triangle should be culled.
      {
    ...
    for (int i = 0; i < tri_size * 3; i++) {
      ...
      if (prv >= vert_size) {
        ...
        for (int j = i; j < tri_size * 3; j++) {
          if (indices[j] == prv) indices[j] = nxt;
          else if (indices[j] == nxt) indices[j] = prv;
        }
  ...
  private static Mesh ConvertNavmeshToMesh(Vector3[] vraw, int[] iraw, int vert_size, int tri_size) {
    ...
    for (int x = 0; x < triangles.Length; x++) triangles[x] = iraw[x];
      ...
      for (int y = x + 1; y < size; y++) {
        ...
        if (x != y && Mathf.Abs(d.x) < threshold && Mathf.Abs(d.y) < threshold && Mathf.Abs(d.z) < threshold) {
          ...
          for (int z = 0; z < elts.Length; z++) {
            if (elts[z] == y) elts[z] = x;

            if (elts[z] == size - 1) elts[z] = y;
          }
          size--;
          y--;
        }
  ...
  private static BorderPointSet[] FindBorderEdges(Mesh m) {
    ...
    while (!finished) {
      ...
      for (int x = 0; x < raw.Length; x++) fmt[x] = m.vertices[raw[x]];
      ret.Add(fmt);
  ...
  private static LayerMask LayerMaskField(string label, LayerMask layerMask) {
    ...
    for (int i = 0; i < layers.Length; i++) layerNumbers.Add(LayerMask.NameToLayer(layers[i]));
