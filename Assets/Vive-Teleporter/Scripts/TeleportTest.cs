...
public class TeleportTest : MonoBehaviour {
  ...
  private float pointer_pitch;
  private float pointer_yaw;
  ...
  private void Update() {
    ...
    if (Input.GetButtonDown("Click") && Pointer.PointOnNavMesh) transform.position = Pointer.SelectedPoint;
  }

  private void OnGUI() {
    GUI.Label(new Rect(Screen.width / 2 - 150, Screen.height - 60, 300, 50),
      "Hold ALT to turn camera, Click to teleport, Mouse to rotate controller/camera", InstructionsStyle);
  }
}
