...
public class ViveNavMesh : MonoBehaviour {
  [SerializeField]
  private readonly NavmeshDewarpingMethod _DewarpingMethod = NavmeshDewarpingMethod.None;

  [SerializeField]
  private readonly bool _IgnoreSlopedSurfaces = true;

  [SerializeField]
  [HideInInspector]
  private readonly int _NavAreaMask = ~0; // Initialize to all

  [SerializeField]
  private readonly float _SampleRadius = 0.25f;
  ...
  [SerializeField]
  private int _QueryTriggerInteraction;
  ...
  public Material GroundMaterial {
    ...
    set {
      ...
      if (_GroundMaterial != null) _GroundMaterial.SetFloat(AlphaShaderID, GroundAlpha);
      if (old != _GroundMaterial) Cleanup();
    }
  }
  ...
  public BorderPointSet[] SelectableMeshBorder {
    ...
    set {
      _SelectableMeshBorder = value;
      Border.Points = _SelectableMeshBorder;
    }
  }

  public bool IgnoreSlopedSurfaces {
    get { return _IgnoreSlopedSurfaces; }
  }
  ...
  public NavmeshDewarpingMethod DewarpingMethod {
    get { return _DewarpingMethod; }
  }

  private void Start() {
    if (SelectableMesh == null) SelectableMesh = new Mesh();
    if (_SelectableMeshBorder == null) _SelectableMeshBorder = new BorderPointSet[0];
    ...
    if (_GroundMaterialSource != null) GroundMaterial = new Material(_GroundMaterialSource);
  ...
  private void OnRenderObject() {
    ...
    if (!_SelectableMesh || !GroundMaterial) return;

    var cam = Camera.current;
    if (!cam || cam.cameraType == CameraType.Preview || ((1 << gameObject.layer) & Camera.current.cullingMask) == 0) return;
    ...
    buf.DrawMesh(_SelectableMesh, Matrix4x4.TRS(Vector3.up * 0.005f, Quaternion.identity, Vector3.one), GroundMaterial,
      0);
    cameras[cam] = buf;
  ...
  private void OnValidate() {
    ...
    if (AlphaShaderID == -1) AlphaShaderID = Shader.PropertyToID("_Alpha");
  }
  ...
  public bool Linecast(Vector3 p1, Vector3 p2, out bool pointOnNavmesh, out Vector3 hitPoint, out Vector3 normal) {
    ...
    dir /= dist;
    if (Physics.Raycast(p1, dir, out hit, dist, _IgnoreLayerMask ? ~_LayerMask : _LayerMask,
