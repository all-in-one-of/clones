...
namespace Valve.VR.InteractionSystem {
  ...
  public class PlaySound : MonoBehaviour {
    [Header("Delay Time")] [Tooltip("Time to offset playback of sound")] public float delayOffsetTime = 0.0f;
    ...
    [Tooltip("Percent chance that the wave file will not play")] [Range(0.0f, 1.0f)] public float percentToNotPlay =
      0.0f;

    [Tooltip("Maximum pitch that will be used when randomly set.")] [Range(-3.0f, 3.0f)] public float pitchMax = 1.0f;

    [Tooltip("Minimum pitch that will be used when randomly set.")] [Range(-3.0f, 3.0f)] public float pitchMin = 1.0f;

    [Tooltip("Start a wave file playing on awake, but after a delay.")] public bool playOnAwakeWithDelay;

    [Tooltip(
      "If the sound is looping and updating it's position every frame, stop the sound at the end of the wav/clip length. "
    )] public bool stopOnEnd;

    [Tooltip("Stops the currently playing clip in the audioSource. Otherwise clips will overlap/mix.")] public bool
      stopOnPlay;

    [Tooltip("Inital time before the first repetion starts")] [Range(0.0f, 360.0f)] public float timeInitial = 0.0f;

    [Tooltip("Maximum pitch that will be used when randomly set.")] [Range(0.0f, 360.0f)] public float timeMax = 0.0f;
    ...
    [Tooltip("Maximum volume that will be used when randomly set.")] [Range(0.0f, 1.0f)] public float volMax = 1.0f;

    [Tooltip("Minimum volume that will be used when randomly set.")] [Range(0.0f, 1.0f)] public float volMin = 1.0f;
    ...
    private void Awake() {
      ...
      if (audioSource.playOnAwake) {
        if (useRetriggerTime) InvokeRepeating("Play", timeInitial, Random.Range(timeMin, timeMax));
        else Play();
      }
    ...
    public void PlayWithDelay(float delayTime) {
      if (looping) Invoke("PlayLooping", delayTime);
      else Invoke("PlayOneShotSound", delayTime);
    }
    ...
    public AudioClip PlayOneShotSound() {
      if (!audioSource.isActiveAndEnabled) return null;

      SetAudioSource();
      if (stopOnPlay) audioSource.Stop();
      if (disableOnEnd) Invoke("Disable", clip.length);
    ...
    public AudioClip PlayLooping() {
      ...
      if (stopOnEnd) Invoke("Stop", audioSource.clip.length);
      return clip;
    }
