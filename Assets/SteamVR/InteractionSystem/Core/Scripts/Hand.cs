...
namespace Valve.VR.InteractionSystem {
  ...
  public class Hand : MonoBehaviour {
    ...
    public const AttachmentFlags defaultAttachmentFlags =
      AttachmentFlags.ParentToHand | AttachmentFlags.DetachOthers | AttachmentFlags.DetachFromOtherHand |
    ...
    private void UpdateHovering() {
      ...
      float boxMult = Util.RemapNumberClamped(handDiff, 0.0f, 0.5f * flHoverRadiusScale, 5.0f, 1.0f) *
                      flHoverRadiusScale;
      ...
      Physics.OverlapBoxNonAlloc(
        ...
        overlappingColliders, Quaternion.identity, hoverLayerMask.value);
      ...
      foreach (Collider collider in overlappingColliders) {
        ...
        if (attachedObjects.FindIndex(l => l.attachedObject == contacting.gameObject) != -1) continue;

        // Occupied by another hand, so we can't touch it
        if (otherHand && otherHand.hoveringInteractable == contacting) continue;
        ...
        if (attachedObjects.Count > 0) {
          ...
          if (Physics.Raycast(ray, out raycastHit, noSteamVRFallbackMaxDistanceNoItem)) {
            ...
            transform.position = ray.origin +
                                 Mathf.Min(noSteamVRFallbackMaxDistanceNoItem, noSteamVRFallbackInteractorDistance) *
                                 ray.direction;
          } else {
    ...
    private void UpdateDebugText() {
      if (showDebugText) {
        ...
        debugText.text =
          string.Format(
            "Hovering: {0}\n" + "Hover Lock: {1}\n" + "Attached: {2}\n" + "Total Attached: {3}\n" + "Type: {4}\n",
            (hoveringInteractable ? hoveringInteractable.gameObject.name : "null"), hoverLocked,
            (currentAttachedObject ? currentAttachedObject.name : "null"), attachedObjects.Count, GuessCurrentHandType());
      } else {
        if (debugText != null) {
    ...
    public override void OnInspectorGUI() {
      ...
      if (hand.otherHand) {
        if (hand.otherHand.otherHand != hand) {
          EditorGUILayout.HelpBox("The otherHand of this Hand's otherHand is not this Hand.", MessageType.Warning);
        }

        if (hand.startingHandType == Hand.HandType.Left && hand.otherHand.startingHandType != Hand.HandType.Right) {
          EditorGUILayout.HelpBox("This is a left Hand but otherHand is not a right Hand.", MessageType.Warning);
        }

        if (hand.startingHandType == Hand.HandType.Right && hand.otherHand.startingHandType != Hand.HandType.Left) {
          EditorGUILayout.HelpBox("This is a right Hand but otherHand is not a left Hand.", MessageType.Warning);
        }
