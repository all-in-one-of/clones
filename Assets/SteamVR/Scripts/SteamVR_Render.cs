...
public class SteamVR_Render : MonoBehaviour {
  ...
  private string GetScreenshotFilename(uint screenshotHandle,
    ...
    if (capacity > 1) {
      ...
      OpenVR.Screenshots.GetScreenshotPropertyFilename(screenshotHandle, screenshotPropertyFilename,
  ...
  private void OnRequestScreenshot(VREvent_t vrEvent) {
    ...
    if (screenshotType == EVRScreenshotType.StereoPanorama) {
      string previewFilename = GetScreenshotFilename(screenshotHandle,
        EVRScreenshotPropertyFilenames.Preview);
      ...
      // and submit it
      OpenVR.Screenshots.SubmitScreenshot(screenshotHandle, screenshotType, previewFilename,
        VRFilename);
    }
  }
  ...
  private void Update() {
    ...
    if (lockPhysicsUpdateRateToRenderFrequency && Time.timeScale > 0.0f) {
      ...
      if (vr != null) {
        ...
        timing.m_nSize =
          (uint) Marshal.SizeOf(typeof(Compositor_FrameTiming));
