...
public class SteamVR : IDisposable {
  ...
  public static bool enabled {
    get {
      if (!UnityEngine.XR.XRSettings.enabled) enabled = false;
      return _enabled;
    }
    set {
      _enabled = value;
  ...
  public static bool usingNativeSupport {
    get { return UnityEngine.XR.XRDevice.GetNativePtr() != IntPtr.Zero; }
  }
  ...
  public string GetTrackedDeviceString(uint deviceId) {
    ...
    var capacity = hmd.GetStringTrackedDeviceProperty(deviceId, ETrackedDeviceProperty.Prop_AttachedDeviceId_String,
      null, 0, ref error);
    if (capacity > 1) {
      var result = new StringBuilder((int) capacity);
      hmd.GetStringTrackedDeviceProperty(deviceId, ETrackedDeviceProperty.Prop_AttachedDeviceId_String, result, capacity,
        ref error);
  ...
  private void OnNewPoses(TrackedDevicePose_t[] poses) {
    ...
    if (poses.Length > OpenVR.k_unTrackedDeviceIndex_Hmd) {
      ...
      var calibrating = result == ETrackingResult.Calibrating_InProgress ||
