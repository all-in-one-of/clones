...
public class SteamVR_Skybox : MonoBehaviour {
  ...
  public static void SetOverride(Texture front = null, Texture back = null, Texture left = null, Texture right = null,
                                 Texture top = null, Texture bottom = null) {
    ...
    if (compositor != null) {
      ...
      if (error != EVRCompositorError.None) {
        ...
        if (error == EVRCompositorError.TextureIsOnWrongDevice) Debug.Log("Set your graphics driver to use the same video card as the headset is plugged into for Unity.");
        else if (error == EVRCompositorError.TextureUsesUnsupportedFormat) Debug.Log("Ensure skybox textures are not compressed and have no mipmaps.");
      }
    }
  }
